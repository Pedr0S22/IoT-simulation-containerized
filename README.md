...
## Project Structure
The architecture relies on network separation, with one network for database communications (`database-network`) and another for the message broker (`broker-network`). Services must communicate using their DNS names (e.g., rabbitmq, mariadb), not IP addresses.

![System Architecture](architecture-iot-simulation.png) 
...
## Database Schema
The MariaDB container should automatically create a database named `sic` upon startup. The Python scripts must create the following tables if they do not already exist:
...
### Simulator Variables
* `NUM_MESSAGES`: Controls the number of messages to generate.
* `N_TEMP_HIGH` / `N_TEMP_LOW`: Sets the normal temperature range.
* `A_TEMP_HIGH` / `A_TEMP_LOW`: Sets the anomaly temperature range.
* `A_PROBA`: Configures the probability of an anomaly occurring.

### Processor Variables
* `TEMP_THRESHOLD`: The temperature threshold for generating alerts.

*(Additional necessary data to perform different tasks, e.g., connect to the DB, can also be supplied via environment variables.)*
